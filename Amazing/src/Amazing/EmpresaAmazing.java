...
//		if (cuit.equals("empty")) {
//			throw new RuntimeException("No se puede crear empresa vacia");
//		}
		this.cuit = cuit;
...
//		if (!getTransporte(patente).estaCargado()) {
...
//	    if (getTransporte(patente).devolverTamCarga()) {
//	        
//	        return "Transporte no cargado";
//	    }
		...
		for (Integer cod:pedidos.keySet()) {
		     ...
		     if (pedido.devolverEstado() && pedido.devolverCarrito() != null) {
		    	 ...
		    	 for (int codPaq : pedido.devolverCarrito().keySet()) {
		        	 if (pedido.getPaquete(codPaq).devolverEstado().equals("pendiente") && transporte.coincideRequisitos(pedido.getPaquete(codPaq)) && volumenTotalCargado < volumenMaximoTransporte) {

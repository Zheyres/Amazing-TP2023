...
public class Pedido {
	...
	Pedido(int codPedido, String nombre, String direccion, int dni) {
		...
		this.cerrado = false;
		this.costoTotal = 0;
		this.carrito = new HashMap<Integer, Paquete>();
	}
	...
	public boolean eliminarProducto(int codProd) {
		if (carrito.containsKey(codProd) && devolverEstado() == false) {
			this.costoTotal -= getPaquete(codProd).devolverPrecio();
	...
	public HashMap<Integer, Paquete> devolverCarrito() {
		return carrito;
	}
	...
	public boolean tienePaquetesNoEntregados() {
		...
		for (int cod:carrito.keySet()) {
			if (getPaquete(cod).devolverEstado() == false) {
				acum = acum || true;
			}
		}
		return acum;
